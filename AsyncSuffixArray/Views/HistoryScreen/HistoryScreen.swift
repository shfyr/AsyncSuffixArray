...
struct HistoryScreen: View {
    @StateObject var jobQueue = JobQueue()
    @State private var searchHistory: [String] = [] // Локальный массив для хранения истории
    ...
    private func getExecutionTimeColor(for text: String) -> Color {
        if let job = jobQueue.getAllJobs().first(where: { $0.text == text }),
           let duration = job.duration {
            ...
            // Интерполяция от зеленого (min) к красному (max)
